...
func NewLlamaServer(systemInfo ml.SystemInfo, gpus []ml.DeviceInfo, modelPath string, f *ggml.GGML, adapters, projectors []string, opts api.Options, numParallel int) (LlamaServer, error) {
	...
	if envconfig.MoePrefetch() && envconfig.MoePinned() && moeSplitConfigured() {
		if runnerEnvs == nil {
			runnerEnvs = map[string]string{}
		}
...
func moePrefetchReserve(moeSize []uint64, moeLayers []int) uint64 {
	if !envconfig.MoePinned() || !envconfig.MoePrefetch() {
		return 0
	}
	var maxSize uint64
	for _, layer := range moeLayers {
...
func (s *llmServer) buildLayout(systemGPUs []ml.DeviceInfo, memory *ml.BackendMemory, requireFull bool, backoff float32) (gpuLayers ml.GPULayersList, denseGPULayers ml.GPULayersList, layers []uint64, err error) {
	...
	if isMoEModel && moeSplitConfigured() {
		requestedMoEGPULayers, source, err := requestedMoEGPULayers(len(layers), moeExpertLayers)
		if err != nil {
		...
		prefetchReserve := moePrefetchReserve(moeSize, moeExpertLayers)
		...
		if totalDenseAndPrefetch > availableVRAM {
			...
			if source == "auto" {
				...
				moeGPUCount := 0
				for _, layer := range moeExpertLayers {
				...
				requestedMoEGPULayers = slices.Clone(moeExpertLayers[len(moeExpertLayers)-moeGPUCount:])
			}

			slog.Info("moe split: layer budget",
				"moe_layers", len(moeExpertLayers),
				"moe_gpu_layers", len(requestedMoEGPULayers),
				"moe_cpu_layers", len(moeExpertLayers)-len(requestedMoEGPULayers),
				"cfg", source)

			intendedMoEGPUSet := moeGPUSetFromLayers(requestedMoEGPULayers)
			for i := range layers {
				loc := "cpu"
			...
			gpuLayersMoE := findBestFit(adjustedLayers, adjustedGPUs, len(requestedMoEGPULayers), false)
			gpuLayersMoE = remapMoELayers(gpuLayersMoE, moeExpertLayers)
			if gpuLayersMoE.Sum() != len(requestedMoEGPULayers) {
				if source != "auto" {
					return nil, nil, nil, fmt.Errorf("moe split cannot allocate requested %d GPU MoE layers; allocated %d", len(requestedMoEGPULayers), gpuLayersMoE.Sum())
				}
				requestedMoEGPULayers = requestedMoEGPULayers[:gpuLayersMoE.Sum()]
			}
